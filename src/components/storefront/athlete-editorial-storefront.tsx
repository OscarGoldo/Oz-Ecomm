  ...
  const nodes: Record<string, ReactNode> = {
    "lo-nuevo":
      featured.length > 0 && loNuevo.enabled ? (
        <section key="lo-nuevo" className="border-b py-12">
    ...
    about:
      theme.about.text && about.enabled ? (
        <section key="about" className="border-t py-16">
      ...
      {!hasFilters && (
        <section className="relative flex min-h-[85vh] items-end overflow-hidden">
          ...
          <div className="container relative pb-14 text-white sm:pb-20">
            ...
            {hero.subtext && (
              <p className="mt-4 max-w-sm text-sm font-light text-white/75">{hero.subtext}</p>
            )}
            <a
              href="#catalogo"
